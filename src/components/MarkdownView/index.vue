...
const { copy } = useClipboard() // Initialize the copying to the clipboard.
...
const md = new MarkdownIt({
  highlight: function (str, lang) {
    if (lang && hljs.getLanguage(lang)) {
      try {
        const copyHtml = `<div id="copy" data-copy='${str}' style="position: absolute; right: 10px; top: 5px; color: #fff;cursor: pointer;">copy</div>`
...
.markdown-view {
  ...
  ol > li {
    ...
    margin-bottom: 1rem;
    // expression,Fix the issue of incomplete display of sequential table numbers
    // &:nth-child(n + 10) {
    //     margin-left: 30px;
    // }
