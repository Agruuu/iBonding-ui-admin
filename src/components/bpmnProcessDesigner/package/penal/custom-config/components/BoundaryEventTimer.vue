<template>
  <div>
    ...
    <el-form-item label="Execute Action" prop="timeoutHandlerType" v-if="timeoutHandlerEnable">
...
const resetElement = () => {
  ...
  // Do you want to enable custom user task timeout handling
...
const timeoutHandlerChange = (val) => {
  ...
  if (val) {
    // Enable custom user task timeout handling
    // Boundary event type --- overtime
    boundaryEventType.value = bpmnInstances().moddle.create(`${prefix}:BoundaryEventType`, {
      value: 1
    })
    configExtensions.value.push(boundaryEventType.value)
    // Timeout processing type
    ...
    // Disable custom user task timeout handling
